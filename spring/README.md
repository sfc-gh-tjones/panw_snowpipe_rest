...
* `panw_rest_api.buffer_manager_max_buffer_row_count` - the maximum row count that should be buffered in memory for a
  given table
* `panw_rest_api.max_client_lag` - the Client lag that should be passed through to the Snowpipe Streaming Client SDK
* `panw_rest_api.drain_manager_num_threads` - the number of threads that should be used to drain data from in-memory
  buffers
* `panw_rest_api.drain_manager_max_duration_to_drain_ms` - the maximum a given thread should drain before returning the
  thread to the thread pool
* `panw_rest_api.drain_manager_max_records_to_drain` - the maximum number of records a given thread should drain before
  returning the thread to the thread pool

Note that `panw_rest_api.drain_manager_max_duration_to_drain_ms` and `panw_rest_api.drain_manager_max_records_to_drain`
...
* `PANW_REST_API_DRAIN_MANAGER_MAX_DURATION_TO_DRAIN_MS` for `panw_rest_api.buffer_manager_max_buffer_row_count`
* `PANW_REST_API_MAX_CLIENT_LAG` for `panw_rest_api.max_client_lag`
* `PANW_REST_API_DRAIN_MANAGER_NUM_THREADS` for `panw_rest_api.drain_manager_num_threads`
* `PANW_REST_API_DRAIN_MANAGER_MAX_DURATION_TO_DRAIN_MS` for `panw_rest_api.drain_manager_max_duration_to_drain_ms`
* `PANW_REST_API_DRAIN_MANAGER_MAX_RECORDS_TO_DRAIN` for `panw_rest_api.drain_manager_max_records_to_drain`
